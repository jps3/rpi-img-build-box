...
log "    # SALTSTACK_PY_VERSION : '${SALTSTACK_PY_VERSION}'"
...
SALTSTACK_REPO_URL="https://repo.saltstack.com/\${SALTSTACK_PY_VERSION}/\${ID_LIKE}/\${VERSION_ID}/armhf/\${SALTSTACK_VERSION}"
if ! (dpkg-query --show salt-minion); then 
  wget -O - \${SALTSTACK_REPO_URL}/SALTSTACK-GPG-KEY.pub | sudo apt-key add -
  echo "deb \${SALTSTACK_REPO_URL} \${VERSION_CODENAME} main" | tee /etc/apt/sources.list.d/saltstack.list
  apt-get update -y
